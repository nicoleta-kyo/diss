# -*- coding: utf-8 -*-
"""
Created on Mon May 11 01:03:07 2020
...
class LettersEnv():
  ...
  def __init__(self, states=None, num_states=None, max_cost=None, goal_state=None):
      ...
      self.t = 0
      self.num_act = 4
      self.num_combs = np.max(self.states.index.values)+1  # number of combinations
      if goal_state is None:
          self.goal_state = np.max(self.states.index.values)   # default goal state is the one of highest cost
  ...
  def _return_state(self,cost):
      ...
      if (np.unique(self.ssm[cost-1, self.t+1-cost:self.t]) == -1).all():
          ...
          if not self.states[self.states.bitseqint == act_tomatch].empty:
              matched_str = self.states[self.states.bitseqint == act_tomatch].state
...
#      if self.goal_state in obs:
  ...
  def gen_states(self, num_states, max_cost, short=True):
      
      self.max_cost = max_cost
      states = gen_base()
      stns = gen_state_names(short)
