...
- Embodiment for environment Letters
...
#         pdb.set_trace()
    ...
    def connectToEnv(self, states, max_cost=None, goal_state=None):
        self.env = LettersEnv(states, max_cost, goal_state)
...
#         pdb.set_trace()
        ...
        #binarise action
        action_bit =  inputToBit(action, self.maxact, self.Msize)
    ...
    def SarsaLearning(self, total_episodes, max_steps, use_rew, Beta=None, gamma=None, lr=None):
...
#         pdb.set_trace()
        ...
        for episode in range(total_episodes):
            ...
            beta = Beta[episode] if Beta is not None else self.defaultBeta
...
#                esn_input = np.hstack([state_bit, action_bit]).reshape(1,-1)
                esn_input = np.hstack([inputToBit(state, self.maxobs, self.Ssize), inputToBit(action, self.maxact, self.Msize)]).reshape(1,-1)
                ...
                # !!!! reward hard-coded
                esn_inhist = np.hstack([esn_input, inputToBit(state2, self.maxobs, self.Ssize).reshape(1,-1), inputToBit(ext_reward, 2, 2).reshape(1,-1)])
                # Predictor improvement and calculation of internal reward
                self.predictor.history[episode,t,:] = esn_inhist  # update predictor history
                int_reward, qualp = self.predictor.calculateInternalReward() # calculate internal reward
          
                # Calculation of reward for SARSA update
                if use_rew == 'int':        
                    reward = int_reward
                elif use_rew == 'int+ext':
            ...
            self.log[episode, :] = np.array([state, ext_reward, round(np.mean(PredQual[:t]),2), np.mean(Qs[:t]), round(np.mean(int_rew[:t]),2),
                    round(np.mean(np.abs(vishidJ)),3), round(np.max(np.abs(vishidJ)),3)])
...
#         pdb.set_trace()
        ...
        for a in range(self.env.num_act):
            if -beta*self.CalcFreeEnergy(state, a) > 700:
                bfe = 700
                print('-beta*FE in chooseAction reached maximum value for exp.')
            else:
                bfe = -beta*self.CalcFreeEnergy(state, a)
            fa = np.exp(bfe)
            fa_ = 0
            for a_ in range(self.env.num_act):
                if -beta*self.CalcFreeEnergy(state, a_) > 700: 
                    bfe = 700 
                    print('-beta*FE in chooseAction reached maximum value for exp.')
                else:
                    bfe = -beta*self.CalcFreeEnergy(state, a_)
                fa_ = fa_ + np.exp(bfe)
            p_a[a,1] = fa/fa_
    ...
    def displayRunData(self, total_episodes, num_el):
        ...
        fig, ax = plt.subplots()
        line1, = ax.plot(x, y, label='external reward')
