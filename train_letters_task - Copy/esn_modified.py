...
from cannon_rlsfilter import RLSFilterAnalyticIntercept, RecursiveLassoFilter
import math
import warnings
import pdb
...
def atanh(x):
    ...
    atanhx = np.zeros((1,x.shape[1]))
    for i,v in enumerate(x[0]):
        atanhx[:,i] = math.atanh(v-np.finfo('float').eps)
...
def MAE(y,p):
    return round(np.mean(np.abs((p - y)),4))
...
class ESN():
    ...
    def initweights(self):
        ...
        if self.lassorls is False:
            ...
            self.RLSfilter = RecursiveLassoFilter(self.n_reservoir+self.n_inputs+self.input_bias, self.n_outputs, forgetting_factor=0.995)
...
#         pdb.set_trace()
    ...
    def setHistory(self, episodes, steps):
        ...
        self.history = np.repeat(float('inf'), episodes*steps*self.num_elem_hist).reshape(episodes, steps, self.num_elem_hist)
...
#        teachers = self.inverse_out_activation(teachers)
        ...
        preds1 = np.zeros((inputs.shape[0], self.n_outputs), dtype='float64')
        ...
        # calculate predictor 
        quality1 = np.mean(np.abs((preds1 - teachers)))
        
        
        #--------- update filter with last input-output
        self.RLSfilter.process_datum(res_states[-1,:].reshape(-1,1), self.inverse_out_activation(teachers[-1,:]).reshape(-1,1))
        
        #------- calc C(p_new, hist)
        preds2 = np.zeros((inputs.shape[0], self.n_outputs), dtype='float64')
        for i in range(inputs.shape[0]):
            preds2[i,:] = self.out_activation(self.RLSfilter.predict(res_states[i,:].reshape(-1,1)).T)  
    
        # calculate predictor quality and save it
        self.quality = np.mean(np.abs((preds2 - teachers)))
