# -*- coding: utf-8 -*-
"""
Load non-crit RBM and non-crit ESN results
...
# --------------------------------------------------- Non-crit RBM - non-crit ESN results
...
# 5453 - after this the actor starts executing only A and the predictor goes very low
# 5597    # after that predictor error went very high, leading to more rewards received and only action 0 was performed again
eps = 5453  
#allst = 14
...
#plot_vis_st3 = np.zeros(vals)
#for i in range(vals):
#    if i==(vals-1):
#        plot_vis_st3[i] = np.mean(rst2[50*i,:neweps])
#    plot_vis_st3[i] = np.mean(rst2[50*i,:50*(i+1)])
...
#
#
##----------------------------------------II. mean pred quality per episode 
#
#
...
##res[''] = intrew_ep
##
##filetosave = 'runData\\res\\perf_AnonPnon.pkl'
##
...
#
#filetosave = 'runData\\res\\perf_AnonPnon_compSt.pkl'
#
#if not os.path.isfile(filetosave):
#    with open(filetosave, 'wb') as output:
#            pickle.dump(res, output, pickle.HIGHEST_PROTOCOL)
