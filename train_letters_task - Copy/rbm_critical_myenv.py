...
class ising:
    ...
    def __init__(self, netsize, Nmemory, Nreward, Nsensors, Nmotors, predictor, states, numsteps):  # Create ising model
...
#         pdb.set_trace()
        ...
        self.connectToEnv(states, numsteps)
    ...
    def connectToEnv(self, states, numsteps):
        self.env = LettersEnv(states)
        self.observation = self.env.reset(numsteps)
        self.maxobs = self.env.num_combs
        self.maxact = self.env.num_act
        self.maxrew = 2 # !!!! i think
    ...
    def initialise_sensors(self):
        self.sensors = np.zeros((self.Ssize+self.Memsize+self.Rewsize,1),dtype=int) - 1
        self.sensors[:self.Ssize] = input2bit(self.observation, self.maxobs, self.Ssize) * 2 - 1
    ...
    def randomize_position(self, numsteps):
        self.observation = self.env.reset(numsteps)
...
#         pdb.set_trace()
        ...
        self.log[self.episode, self.t, :] = [qual, int_reward] # store predictor's error after seeing the output of this move 
    ...
    def UpdateSensors(self, state, memory, reward):  
        ...
        state_bit = input2bit(state, self.maxobs, self.Ssize) * 2 - 1 
        self.sensors[:self.Ssize] = state_bit
        # binarise memory
        memory_bit = np.zeros((memory.shape[1],1),dtype=int)
        for i in range(memory.shape[1]):
            memory_bit[i] = -1 if memory[0,i] < 0.5 else 1
        self.sensors[self.Ssize:(self.Ssize+self.Memsize)]= memory_bit
        # binarise reward
        reward_bit = input2bit(reward, self.maxrew, self.Rewsize) * 2 - 1
...
#         pdb.set_trace()
        ...
        numeps = int(Iterations/10)+1
        numsteps = T*10
            
        self.observations = np.tile( np.repeat(-1,  numsteps) , (numeps,1) )    #keep track of reached states
        self.predictor.setHistory(numeps, numsteps) # resetting the env every 10 iterations, so episodes and steps adjusted accordingly 
        self.log = np.tile(np.repeat(-np.inf,2), (numeps, numsteps, 1) )# store pred quality and internal reward for every time step
...
#                 pdb.set_trace()
                ...
                self.randomize_position( T*10 )  # updates self.observation to be initial (-1)
