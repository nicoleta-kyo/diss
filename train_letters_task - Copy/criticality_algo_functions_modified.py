...
def det_criticality(esn_config, inputs, param_space, num_iter, num_trials, num_pert, sigma_p, filenmoutput):
    ...
    for it in range(num_iter):        #different stopping criterion?? for now i'd go through all parameter sets
        ...
        for t in range(num_trials):
            ...
            #----- get activations only for valid time steps
            states = esn.get_states(valid_steps(inputs[0,:,:]), extended=False, continuation=False)
            for ep in range(1,inputs.shape[0]):
                epinputs = valid_steps(inputs[ep,:,:])                       
                statesep = esn.get_states(epinputs, extended=False, continuation=False)
                states = np.vstack([states,statesep])
            #----
            ...
            for j in range(num_pert):
                ...
                #----- get activations only for valid time steps
                st = esn.get_states(valid_steps(inputs[0,:,:]), extended=False, continuation=False)
                for ep in range(1,inputs.shape[0]):
                    epinputs = valid_steps(inputs[ep,:,:])                       
                    step = esn.get_states(epinputs, extended=False, continuation=False)
                    st = np.vstack([st,step])
                #----
        ...
        print('Finished iteration '+ str(it) + ' in ' + str(int((toc - tic)/60)) + ' minutes.')
