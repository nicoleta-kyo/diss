...
def det_criticality(esn_config, narma_in, narma_out, param_space, num_iter, num_trials, num_pert, sigma_p):
    ...
    for iter in range(num_iter):        #different stopping criterion?? for now i'd go through all parameter sets
...
#        timer.tic()
        ...
        #            st = st[1:st.shape[0],:]    # get rid of first zero 
                ...
                print("Computing FR statistic " + str(j+1) + "..." + " from trial " + str(t+1) + "..." + "iteration " + str(iter) + "...")
        ...
        dets[iter] = linalg.det(av_fim)     # store new determinant
        if dets[iter] > det:                # if new determinant > curr best, update curr best
            det = dets[iter]
            param_set_star = param_set
        
        if iter+1 < param_space.shape[0]:
            param_set = param_space[iter+1,:].reshape(d,1)      # choose next param set to evaluate
